...
public class InventoryServiceImpl implements InventoryService {
    ...
    public void reserve(UUID eventId, ReserveCommand command) {
        ...
        List<RequestedItem> sortedItems = command.items().stream()
                .sorted(Comparator.comparing(
                                (RequestedItem i) -> i.resourceType().name())
        ...
        try {
            ...
            log.error("Oversell guard tripped while reserving bookingId={}: {}", command.bookingId(), e.getMessage());
            throw e;
        }
    ...
    private void evaluateFlight(RequestedItem item, List<FlightReservable> reservable, List<FailedItem> failed) {
        Optional<FlightInventory> found = flightInventoryRepository.findForUpdate(item.resourceId());
        if (found.isEmpty()) {
            log.warn(
                    "BookingCreated references unknown flight, treating as unavailable: resourceId={}",
                    item.resourceId());
        ...
        if (!inventory.canReserve(item.quantity())) {
            failed.add(new FailedItem(
                    ResourceType.FLIGHT, item.resourceId(), item.quantity(), Math.max(0, inventory.available())));
            return;
        }
        reservable.add(new FlightReservable(item, inventory));
    }

    private void evaluateHotel(RequestedItem item, List<HotelReservable> reservable, List<FailedItem> failed) {
        List<LocalDate> nights = item.nights();
        List<RoomTypeNightAvailability> rows =
                roomTypeAvailabilityRepository.findForUpdateByRoomTypeIdAndStayDateIn(item.resourceId(), nights);
        ...
        int minAvailable = rows.stream()
                .mapToInt(RoomTypeNightAvailability::available)
                .min()
                .orElse(0);
        boolean allReservable = rows.stream().allMatch(row -> row.canReserve(item.quantity()));
        if (!allReservable) {
            failed.add(
                    new FailedItem(ResourceType.HOTEL, item.resourceId(), item.quantity(), Math.max(0, minAvailable)));
            return;
        }
    ...
    private ReservedItem reserveFlight(
            FlightReservable reservable, ReserveCommand command, Instant expiresAt, long version) {
        reservable.inventory().reserve(reservable.item().quantity());
        meterRegistry
                .counter(M_UNITS, KEY_TAG_ACTION, "reserved")
                .increment(reservable.item().quantity());

        UUID reservationId = UUID.randomUUID();
        reservationRepository.save(new FlightReservation(
                reservationId,
                command.bookingId(),
                reservable.item().resourceId(),
                reservable.item().quantity(),
        ...
        outboxEventWriter.write(
                ...
                new FlightAvailabilityPayload(
                        ...
                        command.bookingId(),
                        reservable.item().resourceId(),
                        reservable.inventory().getReservedCount(),
                        version));
        ...
        return new ReservedItem(
                ...
                reservable.item().resourceId(),
                reservable.item().quantity(),
                reservable.item().amount());
    }

    private ReservedItem reserveHotel(
            HotelReservable reservable, ReserveCommand command, Instant expiresAt, long version) {
        ...
        reservationRepository.save(new HotelReservation(
                reservationId,
                command.bookingId(),
                reservable.item().resourceId(),
                reservable.item().quantity(),
                ...
                command.correlationId(),
                command.sagaId(),
                reservable.item().checkIn(),
                reservable.item().checkOut()));
        recordHistory(reservationId, ReservationStatus.RESERVED);
        ...
        outboxEventWriter.write(
                ...
                new HotelAvailabilityPayload(
                        reservationId, command.bookingId(), reservable.item().resourceId(), hotelId, nights, version));
    ...
    public void confirm(UUID eventId, UUID bookingId) {
        ...
        for (Reservation reservation : reservationRepository.findByBookingId(bookingId)) {
            if (reservation.getStatus() == ReservationStatus.RESERVED) {
                ...
                log.debug(
                        "Ignoring BookingConfirmed for reservation in state {}: reservationId={}",
    ...
    @Transactional
    public void release(
            UUID eventId, UUID bookingId, ConsumerEventType triggerEventType, String correlationId, String sagaId) {
        if (consumedEventRepository.existsById(eventId)) {
        ...
        for (Reservation reservation : reservationRepository.findByBookingId(bookingId)) {
            ...
            ReservationStateTransitionGuard.assertAllowed(reservation.getStatus(), ReservationStatus.RELEASED);
            reservation.setStatus(ReservationStatus.RELEASED);
            recordHistory(reservation.getId(), ReservationStatus.RELEASED);
            restoreAndEmit(reservation, EventType.released(reservation.resourceType()), correlationId, sagaId, version);
        ...
        if (!releasedIds.isEmpty()) {
            outboxEventWriter.write(
                    ...
                    correlationId,
                    sagaId,
                    new InventoryReleasedPayload(bookingId, releasedIds));
        }
    ...
    public void expire(UUID eventId, UUID bookingId, String correlationId, String sagaId) {
        ...
        if (!expiredIds.isEmpty()) {
            outboxEventWriter.write(
                    ...
                    correlationId,
                    sagaId,
                    new InventoryReleasedPayload(bookingId, expiredIds));
        }
    ...
    public void expireReservation(UUID reservationId) {
        ...
        if (reservation.getStatus() != ReservationStatus.RESERVED) {
            log.debug(
                    "Skipping TTL expiry, reservation not RESERVED: reservationId={}, status={}",
                    reservationId,
                    reservation.getStatus());
            return;
        }
        ...
        log.info(
                "Reservation expired by TTL sweep: reservationId={}, bookingId={}",
                reservationId,
     ...
     */
    private void expireOne(Reservation reservation, String correlationId, String sagaId, long version) {
        ...
        restoreAndEmit(reservation, EventType.expired(reservation.resourceType()), correlationId, sagaId, version);
    }
     ...
     */
    private void restoreAndEmit(
            Reservation reservation, EventType eventType, String correlationId, String sagaId, long version) {
        if (reservation instanceof FlightReservation) {
            FlightInventory inventory = flightInventoryRepository
                    .findForUpdate(reservation.getResourceId())
                    .orElseThrow(
                            () -> new InventoryNotFoundException(ResourceType.FLIGHT, reservation.getResourceId()));
            ...
            List<RoomTypeNightAvailability> rows =
                    roomTypeAvailabilityRepository.findForUpdateByRoomTypeIdAndStayDateIn(
                            hotel.getResourceId(), hotel.nights());
            if (rows.isEmpty()) {
    ...
    private void recordDuplicateSkip(ConsumerEventType type) {
        meterRegistry
                .counter(M_SKIPPED, "reason", "duplicate", "event", type.name().toLowerCase())
                .increment();
    }
